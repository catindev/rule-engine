...
function compile(artifacts, options = {}) {
  ...
  assert(isObject(operators), "compile: options.operators must be provided");
  assert(isObject(operators.check), "compile: options.operators.check must be an object");
  assert(isObject(operators.predicate), "compile: options.operators.predicate must be an object");
  ...
  return { registry, dictionaries, operators };
}
...
function validateStepRef(step, registry, scope, scopePipelineId) {
  ...
  if (k === "rule" || k === "condition") {
    ...
    if (!isLibrary) {
      assert(
        a.__pipelineId === scopePipelineId,
