...
function compile(artifacts, options = {}) {
  ...
  assert(
    isObject(operators) && isObject(operators.check) && isObject(operators.predicate),
  ...
  for (const a of artifacts) {
    if (a.type === "pipeline") {
      ...
      if (a.meta !== undefined) {
        assert(isObject(a.meta), `Rule ${where(a)}: meta must be an object if provided`);
      }
    } else if (a.type === "dictionary") {
      // ok
    } else {
      throw new Error(`Unknown artifact type: ${a.type} (id=${a.id}, source=${fileOf(a.id)})`);
    }
  }
  ...
  for (const a of artifacts) {
    if (a.type === "pipeline") {
      ...
      for (const s of a.flow) validateStepRef(s, registry, `pipeline:${a.id} (${fileOf(a.id)})`, scopePipelineId);
    }
    if (a.type === "condition") {
      ...
      for (const s of a.steps) validateStepRef(s, registry, `condition:${a.id} (${fileOf(a.id)})`, scopePipelineId);
    }
  }

  validatePipelineDAG(registry);

  const compiled = { registry, dictionaries, sources, operators };
...
function validateStepRef(step, registry, scope, scopePipelineId) {
  ...
  // - rules & conditions must be local to the current pipeline (exact __pipelineId)
  //   OR come from library.
  // - pipelines are allowed to reference other pipelines freely.
  if (k === "rule" || k === "condition") {
    const isLibrary = a.__scope === "library" || (typeof a.id === "string" && a.id.startsWith("library."));
    if (!isLibrary) {
      const isLocal = typeof a.id === "string" && a.id.startsWith(scopePipelineId + ".");
      assert(
        isLocal,
