...
function loadArtifactsFromDir(rulesDir) {
  assert(typeof rulesDir === "string" && rulesDir.length > 0, "loadArtifactsFromDir: rulesDir must be a non-empty string");
...
function walk(root, dir, artifacts, sources) {
  ...
  for (const e of entries) {
    ...
    const { artifact, sourceMeta } = normalizeFsArtifact({ obj, relPath: rel, fullPath: full });
