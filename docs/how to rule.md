...
# 16. Сборка и проверка правил

После того как правила написаны — их нужно собрать в снэпшот. Это единственный способ убедиться что всё корректно: нет дублей id, все ссылки разрешены, операторы существуют.

```bash
node tools/build-snapshot.js --version 1.0.0 --author you@bank.ru --pretty
```

Если правила содержат ошибки — снэпшот **не создаётся** и выводится полный список проблем:
...
Все ошибки выдаются за один прогон — не нужно исправлять по одной.

# 17. Запуск сервера

**Локально** (dev-режим, правила из папки `rules/`, с hot-reload):

```bash
node server.js
```

При сохранении любого `.json` файла в `rules/` — сервер автоматически перезагружает правила без перезапуска. Если новые правила не компилируются — сервер продолжает работать со старой версией.

**На сервере** (прод/тест, из снэпшота):

```bash
NODE_ENV=production SNAPSHOT_PATH=./snapshot.json node server.js
```

Если `SNAPSHOT_PATH` не задан — сервер падает при старте с понятной ошибкой.
